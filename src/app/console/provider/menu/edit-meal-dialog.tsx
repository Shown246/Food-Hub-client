...
import { updateProviderMeal, UpdateProviderMealPayload } from '../_actions';
...
export function EditMealDialog({
  ...
  useEffect(() => {
    if (meal && open) {
      ...
      setCategoryId(meal.category?.id || '');
      setPreparationTimeMinutes(
        meal.preparationTimeMinutes ? String(meal.preparationTimeMinutes) : ''
      );
      setImageUrl(meal.imageUrl || '');
      setDietaryLabels(meal.dietaryLabels || []);
      setCustomTag('');
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: UpdateProviderMealPayload) => {
      if (!meal) throw new Error('No meal selected for editing');
      return updateProviderMeal(meal.id, payload);
    },
    onSuccess: (res) => {
      if (res.success) {
        ...
        setErrorMessage(res.message || 'Failed to update meal.');
      }
    },
    onError: (err: any) => {
      setErrorMessage(err?.message || 'An unexpected error occurred while updating meal.');
    },
  });
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-6 rounded-3xl bg-card border-border/80 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Category */}
            <div className="space-y-1.5">
              <Label htmlFor="edit-category" className="text-xs font-semibold flex items-center gap-1.5">
            ...
            {/* Price */}
            <div className="space-y-1.5">
              <Label htmlFor="edit-price" className="text-xs font-semibold flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Preparation Time */}
            <div className="space-y-1.5">
              <Label htmlFor="edit-prep-time" className="text-xs font-semibold flex items-center gap-1.5">
            ...
            {/* Image URL */}
            <div className="space-y-1.5">
              <Label htmlFor="edit-image-url" className="text-xs font-semibold flex items-center gap-1.5">
